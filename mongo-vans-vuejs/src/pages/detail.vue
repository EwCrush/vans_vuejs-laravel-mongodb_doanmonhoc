<template>
  ...
  <div class="container">
    <a-tabs class="w-1/2" v-model:activeKey="activeKey">
      ...
      >
      <a-tab-pane key="2" tab="Đánh giá">Content of Tab Pane 2</a-tab-pane>
    </a-tabs>
  </div>
</template>
  ...
  setup() {
    ...
    function handleDeleteComment(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .delete(`http://127.0.0.1:8000/api/comments/${id}`, {headers: { Authorization: `Bearer ${token.access_token}` }})
            .then((response) => {
